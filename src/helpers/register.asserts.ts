...
export async function expectedDateofBirth(page: Page) {
    const dayInput = page.getByLabel('Day');
    const monthInput = page.getByLabel('Month');
    const yearInput = page.getByLabel('Year');
    if ()
    await expect(dayInput).toHaveValue(String(TestData.register.validDOB.day));
    await expect(monthInput).toHaveValue(String(TestData.register.validDOB.month));
    await expect(yearInput).toHaveValue(String(TestData.register.validDOB.year));
} // con thieu mot vai truong hop khac 
...
export async function expectedGenderOptinal(page: Page) {
    const genderOptionalInput = page.locator('#preferred_pronoun');
...
export async function expectedEmail(page: Page) {
    ...
    const expectedEmailInput = isEmail(actualEmailInput)
    await expect(expectedEmailInput).toBe(true)
}
...
export async function expectedPhone(page: Page) {
    ...
    const expectedPhoneInput = isPhone(acutalPhoneInput);
    await expect(expectedPhoneInput).toBe(true)
}
